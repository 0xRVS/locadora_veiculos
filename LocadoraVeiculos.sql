create database LocadoraVeiculo;
use LocadoraVeiculo;
...
select count(*) as quantidadePagamentos, sum(valorTotal) as totalArrecadado
from pagamento where estado = 'Pago';

select Veiculo.modelo, Veiculo.marca, count(LocacaoVeiculo.idLocacao) as vezesAlugado
from veiculo Veiculo join LocacaoVeiculo on Veiculo.IdVeiculo = LocacaoVeiculo.idVeiculo
group by Veiculo.idVeiculo, Veiculo.modelo, Veiculo.marca
order by vezesAlugado desc;

select cl.nome, sum(pg.valorTotal) as valorDevido
from Locacao lc
join Cliente cl on lc.idCliente = cl.idCliente
join Pagamento pg on lc.idPagamento = pg.idPagamento
